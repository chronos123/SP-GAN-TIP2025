...
def cos_k_freq(x, k=1):
    return math.cos(k * x * math.pi)
...
class CropPatch():
    ...
    def ac_coords_from_ratio(self, coord_ratio, proj):
        ...
        #     return math.tanh(coord_ratio)
            ...
            return cos_k_freq(coord_ratio, proj)
        else:
    ...
    def __call__(self, img):
        ...
        if self.random_crop:
            ...
            if self.return_ac_coords:
                if self.config.train_params.coord_num_dir == 1:
                    ...
                    # assert hasattr(self.config.train_params, "no_coord_encode") and self.config.train_params.no_coord_encode, f"config error  self.config.train_params.no_coord_encode { self.config.train_params.no_coord_encode}"
                    ac_coords = torch.FloatTensor([
                        self.ac_coords_from_ratio(xst / (self.input_size - self.patch_size), "raw"),
                        self.ac_coords_from_ratio(yst / (self.input_size - self.patch_size), "sin"),
                        self.ac_coords_from_ratio(yst / (self.input_size - self.patch_size), "cos"),
                        self.ac_coords_from_ratio(yst / (self.input_size - self.patch_size), 2),
                        self.ac_coords_from_ratio(yst / (self.input_size - self.patch_size), 3),
                    ])
                elif self.config.train_params.coord_num_dir in {3, 21}:
                    if hasattr(self.config.train_params, "no_coord_encode") and self.config.train_params.no_coord_encode:
                        ac_coords = torch.FloatTensor([
                            self.ac_coords_from_ratio(xst / (self.input_size - self.patch_size), "raw"),
                            self.ac_coords_from_ratio(yst / (self.input_size - self.patch_size), "raw"),
                            self.ac_coords_from_ratio(yst / (self.input_size - self.patch_size), "raw"),
                        ])
                    else:
                        ac_coords = torch.FloatTensor([
            ...
            if self.return_ac_coords:
                if self.config.train_params.coord_num_dir == 1:
                    ...
                    if hasattr(self.config.train_params, "no_coord_encode") and self.config.train_params.no_coord_encode:
                        ac_coords = torch.FloatTensor([
                            self.ac_coords_from_ratio(xst / (x_size - self.patch_size), "raw"),
                            self.ac_coords_from_ratio(yst / (y_size - self.patch_size), "raw"),
                            self.ac_coords_from_ratio(yst / (y_size - self.patch_size), "raw"),
                        ])
                    else:
                        ac_coords = torch.FloatTensor([
        ...
        if self.return_ac_coords:
            coords_partial = {
                    "p_x_st": xst / (self.input_size),
                    "p_x_ed": (xst + self.patch_size - 1) / (self.input_size),
                    "p_y_st": yst / (self.input_size),
                    "p_y_ed": (yst + self.patch_size - 1) / (self.input_size),
                    "circular_flag": False,
                    "x_total": (self.input_size),
                    "y_total": (self.input_size),
                    "y_st": yst,
                    "y_ed": yst + self.patch_size,
            }
            return cropped, ac_coords, coords_partial
        else:
            return cropped, None
...
class RectCropPatch():
    ...
    def __call__(self, img):
        assert img.size == self.input_size, f"image size is {img.size} width: {img.width}, input size is {self.input_size}"
        if self.skip_cropping:
            return img, None

        if self.random_crop:
            xst = safe_randrange(0, self.input_size[1] - self.patch_size[1])
            yst = safe_randrange(0, self.input_size[0] - self.patch_size[0])
            if self.return_ac_coords:
                if self.config.train_params.coord_num_dir == 1:
                    ac_coords = torch.FloatTensor([
                        self.ac_coords_from_ratio(xst / (self.input_size[1] - self.patch_size[1] - 1), "raw"),
                    ])
                elif self.config.train_params.coord_num_dir == 2:
                    ac_coords = torch.FloatTensor([
                        self.ac_coords_from_ratio(yst / (self.input_size[0] - self.patch_size[0] - 1), "raw"),
                        self.ac_coords_from_ratio(yst / (self.input_size[0] - self.patch_size[0] - 1), "raw"),
                    ])
                elif self.config.train_params.coord_num_dir == 4:
                    ac_coords = torch.FloatTensor([
                    ...
                    raise ValueError("Unknown coord_num_dir {}".format(self.coord_num_dir))
                partial_x_y = (xst / (self.input_size[1] - self.patch_size[1] - 1), yst / (self.input_size[0] - self.patch_size[0] - 1))
        else: # center crop
            x_size, y_size = img.height, img.width
            if x_size == self.patch_size:
                xst = 0
            else:
                xst = (x_size - self.patch_size) // 2
            ...
            if self.return_ac_coords:
                if self.config.train_params.coord_num_dir == 1:
                    ...
                    raise ValueError("Unknown coord_num_dir {}".format(self.coord_num_dir))
                partial_x_y = (xst / (x_size - self.patch_size), yst / (y_size - self.patch_size))

        cropped = img.crop((yst, xst, yst + self.patch_size[0], xst + self.patch_size[1]))

        if self.return_ac_coords:
            return cropped, ac_coords, partial_x_y
        else:
            raise ValueError("must return ac_coords in this case")
...
class MultiResolutionDataset(Dataset):
    def __init__(self, split=None, img_dir=None, config=None, is_training=True, 
        ...
        if hasattr(config.train_params, "random_crop"):
            if config.train_params.random_crop:
                if is_training: 
                    self.transform = transforms.Compose(
                        pre_resize_op + [
                            transforms.RandomCrop(raw_resize_size),
                    ...
                    self.transform = transforms.Compose(
                        pre_resize_op + [
                            transforms.CenterCrop(raw_resize_size),
    ...
    def __getitem__(self, index):
        ...
        if self.simple_return_full:
            ...
            patch, ac_coords, coords_partial = self.crop_fn(full_img)
            data_pack = dict(patch=patch, full=full_img, coords_partial=coords_partial)
        else:
        ...
        not_img_keys = {"ac_coords", "coords_partial"}
        data_pack = {
