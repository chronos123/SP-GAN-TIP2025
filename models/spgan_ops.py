...
from models.spherenet import GridGeneratorPatchCoordsFixBorder, GridSamplerNewTexture
...
class SphereModulatedConv2d(nn.Module):
    def __init__(
        self,
        in_channel,
        out_channel,
        ...
        self.grid = None
        self.sampler = GridSamplerNewTexture()
        ...
        if style.ndim == 2:
            ...
            if self.upsample:
                ...
                if self.deal_coords:
                    ...
                    input = input.view(1, batch * 256, input.shape[2], input.shape[3])
            ...
            if self.upsample:
                ...
                input_st = input_st.view(1, batch * in_channel, height, width)
                ...
                if not j_test:
                    ...
                    grid = grids.to(input_st.device)
                    grid.requires_grad = False
                else:
                    with torch.no_grad():
                        self.grid_shape = (H, W)
                        self.genSamplingPattern(H, W, stride=1)
                        grid = self.grid.repeat((B, 1, 1, 1)).to(input_st.device)  # (B, H*Kh, W*Kw, 2)
                        grid.requires_grad = False
                         
                input_st = self.sampler(input_st, grid)
                out = F.conv2d(input_st, weight, padding=self.padding, groups=batch, stride=self.sp_k_size)
                _, _, height, width = out.shape
                out = out.view(batch, self.out_channel, height, width)
                if self.demodulate and (self.padding == 0):
                    demod = demod[:, :, self.dirty_rm_size[0]:-self.dirty_rm_size[0], self.dirty_rm_size[1]:-self.dirty_rm_size[1]]
...
class StyledConv(nn.Module):
    def __init__(
        ...
        self.conv = SphereModulatedConv2d(
            in_channel,
            out_channel,
            kernel_size,
            style_dim,
