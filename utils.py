...
def auto_batched_inference_datasetv2(g_ema, config, partition_keys, *args, **kwargs):
    assert len(args) == 0
    batch_size = config.train_params.batch_size
    n_samples = config.log_params.n_save_sample
    # add log partial_xy
    x_st = np.random.rand(batch_size)
    y_st = np.random.rand(batch_size)
    partial_xy = (torch.from_numpy(x_st), torch.from_numpy(y_st))

    kwargs["partial_xy"] = partial_xy
...
def auto_batched_inference(g_ema, config, partition_keys, *args, **kwargs):
    ...
    for ret in all_ret:
        if "coords_partial" in ret.keys():
            ret.pop("coords_partial")
...
def load_state_dict(model, state_dict, mode="normal"):
    pretrained_indexes = []
    model_indexes = []
    new_state_dict = OrderedDict()
    if mode == "normal":
        for i,  j in zip([0, 1, 2, 3], [1, 3, 4, 5]):
            pretrained_indexes.extend([
                f'structure_synthesizer.implicit_model.conv_stack.{i}.conv.conv.weight',
                f'structure_synthesizer.implicit_model.conv_stack.{i}.conv.conv.modulation.weight',
                f'structure_synthesizer.implicit_model.conv_stack.{i}.conv.conv.modulation.bias',
                f'structure_synthesizer.implicit_model.conv_stack.{i}.conv.activate.bias',
            ])
            model_indexes.extend([
                f'structure_synthesizer.implicit_model.conv_stack.{j}.conv.conv.weight',
                f'structure_synthesizer.implicit_model.conv_stack.{j}.conv.conv.modulation.weight',
                f'structure_synthesizer.implicit_model.conv_stack.{j}.conv.conv.modulation.bias',
                f'structure_synthesizer.implicit_model.conv_stack.{j}.conv.activate.bias',
            ])
    elif mode == "final":
        for i,  j in zip([0, 1, 2, 3], [1, 3, 5, 7]):
            pretrained_indexes.extend([
                f'structure_synthesizer.implicit_model.conv_stack.{i}.conv.conv.weight',
                f'structure_synthesizer.implicit_model.conv_stack.{i}.conv.conv.modulation.weight',
                f'structure_synthesizer.implicit_model.conv_stack.{i}.conv.conv.modulation.bias',
    ...
    k_pre_2_model = {
        e1: e2 for e1, e2 in zip(pretrained_indexes, model_indexes)
    }
    for k, v in state_dict.items():
        if k in pretrained_indexes:
            new_state_dict[k_pre_2_model[k]] = v
        else:
            new_state_dict[k] = v
    ...
    if isinstance(model, nn.DataParallel) or isinstance(model, DDP):
        ...
        try:
            ...
            print(f'Pretrained network class {model.__class__.__name__} has excessive layers; Only loading layers that are used')
        except:
            print(f'Pretrained network class {model.__class__.__name__} has fewer layers; The following are not initialized:')
            not_initialized = set()
...
def load_state_dict_baseline(model, state_dict):
    pretrained_indexes = []
    model_indexes = []
    new_state_dict = OrderedDict()
    for i,  j in zip([0, 1, 2, 3], [1, 3, 5, 7]):
