...
class BaseTestManager():
    ...
    def __init__(self, g_ema, device, save_root, config, save_process=False):
        ...
        self.coord_handler = self.g_ema_module.structure_synthesizer.coord_handler
        # g_ema中建立的coordhandler就带有了train_config和test_config的所有信息
        ...
        # Get the image space / latent space step size, 计算重叠大小, 通过两倍图像与一倍图像计算
    ...
    def maybe_parallel_inference(self, testing_vars, g_ema_kwargs=None, index_tuple=None, 
        ...
        if self.enable_parallel_batching:
            ...
            # 超出边界时进行循环赋值
            testing_vars.meta_img = self._circular_assign_value_width(
                testing_vars.meta_img,  
                testing_vars.meta_img.shape[3],
                img_x_st,
                img_x_ed,
                img_y_st,
                img_y_ed,
            ...
            if self.save_process:
                pad_h = 229 // 2
                pad_w = 0 // 2
                for i in range(testing_vars.meta_img.shape[0]):
                    save_dir = f"{self.save_root}/{i}"
                    os.makedirs(save_dir, exist_ok=True)
                    meta_img = testing_vars.meta_img.clamp(-1, 1).permute(0, 2, 3, 1)
                    meta_img = (meta_img + 1) / 2
                    meta_img_np = meta_img[i, pad_h:pad_h+256, pad_w:pad_w+768, :].numpy()
                    plt.imsave(f"{save_dir}/all_{img_x_st}_{img_y_st}.png", meta_img_np)

        return exec_time, flops

    def _circular_assign_value_width(self, tensor:torch.Tensor, y_width, x_st, x_ed, y_st, y_ed, assigin_value):
        """
        给超出边界的值赋值到合适的位置
        """
        if y_ed <= y_width:
            # 不超出循环的情况
            tensor[:, :, x_st:x_ed, y_st:y_ed] = assigin_value
            return tensor
        elif y_ed <= y_width * 2:
            # 超出循环一倍以内的情况
            if y_st < y_width:
                y_ed = y_ed % y_width
                divided = y_width - y_st
                tensor[:, :, x_st:x_ed, y_st:] = assigin_value[:, :, :, :divided]
                tensor[:, :, x_st:x_ed, :y_ed] = assigin_value[:, :, :, divided:]
                return tensor
            else:
                y_st = y_st % y_width
                y_ed = y_ed % y_width
            ...
            y_st = y_st - y_width
            y_ed = y_ed - y_width
            return self._circular_assign_value_width(tensor, y_width, x_st, x_ed, y_st, y_ed, assigin_value)
...
class BaseEditTestManager():
    # 可以自定义对大图进行多少的 center crop, 之后生成小图
...
class BaseTestManagerNoOverlap():
    # 无重叠的test使用
    def __init__(self, g_ema, device, save_root, config):
        self.g_ema = g_ema.eval()
        self.device = device
        ...
        # Get the image space / latent space step size, 计算重叠大小, 通过两倍图像与一倍图像计算
        self.latentspace_step_size = 35
        # pixelspace_disp_unit = outfeat_disps[-1] // ts_input_size
        self.pixelspace_step_size = self.latentspace_step_size
        # self.pixelspace_step_size  = (self.outfeat_sizes_list[-1] // pixelspace_disp_unit) * pixelspace_disp_unit
        # self.latentspace_step_size = self.pixelspace_step_size // pixelspace_disp_unit
        ...
        # Consequently, also records the step sizes in the feature space (output-side) for further noise/latent handling
        # self.outfeat_step_sizes = self.latentspace_step_size
        outfeat_disp_units = outfeat_disps // ts_input_size
        # self.outfeat_step_sizes = self.latentspace_step_size * outfeat_disp_units
        self.outfeat_step_sizes = np.array([i for i in self.outfeat_sizes_list])
...
class BaseTestManagerNearStep():
    ...
    def __init__(self, g_ema, device, save_root, config):
        ...
        # Get the image space / latent space step size, 计算重叠大小, 通过两倍图像与一倍图像计算
        pixelspace_disp_unit = outfeat_disps[-1] // ts_input_size
        self.pixelspace_step_size  = pixelspace_disp_unit * 2
        # 隐层的步长为1, 图像层的步长为16, 配置中的ts_input_size决定了图像层，特征词的单位步长

        self.latentspace_step_size = self.pixelspace_step_size // pixelspace_disp_unit
        print(f"************************** latentspace_step_size is {self.latentspace_step_size} ******************************")
    ...
    def maybe_parallel_inference(self, testing_vars, g_ema_kwargs=None, index_tuple=None, 
        ...
        if self.enable_parallel_batching:
            ...
            # meta_img = testing_vars.meta_img.clamp(-1, 1).permute(0, 2, 3, 1)
            # meta_img = (meta_img + 1) / 2
            # meta_img_np = meta_img[0, :, :, :].numpy()
            # plt.imsave(f"debug_images/all_{img_x_st}_{img_y_st}.png", meta_img_np)
...
class BaseTestManagerNewInterval():
    ...
    def __init__(self, g_ema, device, save_root, config, save_process=False):
        ...
        pixelspace_disp_unit = outfeat_disps[-1] // ts_input_size
        self.pixelspace_step_size  = (self.outfeat_sizes_list[-1] // pixelspace_disp_unit) * pixelspace_disp_unit
        self.ori_latentspace_step_size = self.pixelspace_step_size // pixelspace_disp_unit
        self.latentspace_step_size = self.__interval__
        ...
        # Consequently, also records the step sizes in the feature space (input-side) for further noise/latent handling
        infeat_disp_units = infeat_disps // ts_input_size
        self.ori_infeat_step_sizes = self.ori_latentspace_step_size * infeat_disp_units

        # Consequently, also records the step sizes in the feature space (output-side) for further noise/latent handling
        outfeat_disp_units = outfeat_disps // ts_input_size
        self.ori_outfeat_step_sizes = self.ori_latentspace_step_size * outfeat_disp_units
