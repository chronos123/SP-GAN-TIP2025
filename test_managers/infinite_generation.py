...
from test_managers.base_test_manager import BaseTestManager, BaseTestManagerNearStep
...
class InfiniteGenerationManagerConstantGlobal(BaseTestManager):
    ...
    def create_vars(self, inv_records=None, inv_placements=None):
        ...
        #     during the inference. However, the author is just lazy and abusing his 
        #     CPU memory OuO
        global_latent = self.latent_sampler.sample_constant_global_latent(
            self.config.train_params.batch_size, mixing=mixing, device=self.device)
        self.logger.info(f"global latent is {global_latent}")
        full_local_latent_shape = (
            # Does not account GNN padding here, it is handled within the latent_sampler
            int(self.g_ema_module.calc_in_spatial_size(self.meta_height, include_ss=False)),
            int(self.g_ema_module.calc_in_spatial_size(self.meta_width, include_ss=False)),
...
class InfiniteGenerationManagerConstantGlobalAndLocal(BaseTestManager):
    ...
    def create_vars(self, inv_records=None, inv_placements=None):
        ...
        local_latent = self.latent_sampler.sample_constant_local_latent(
            self.config.train_params.batch_size, 
            device="cpu", # Store in CPU anyway, it can be INFINITLY LARGE!
            specific_shape=full_local_latent_shape)
...
class InfiniteGenerationManagerConstantInput(BaseTestManager):
    # 输入的global, local, coord都相同, 只有noise不同
    def task_specific_init(self, output_size=None):
    ...
    def create_vars(self, inv_records=None, inv_placements=None):
        ...
        meta_coords = self.coord_handler.sample_constant_coord_grid(
            local_latent, 
            is_training=False) # 通过local_latent得到坐标
...
class InfiniteGenerationManagerConstantInputAndNoise(BaseTestManager):
    # 输入的global, local, coord都相同, noise也相同
    constant_noises = None
    def task_specific_init(self, output_size=None):
    ...
    def create_vars(self, inv_records=None, inv_placements=None):
        ...
        # Create randomized noises, 得到随机噪声
        if self.constant_noises is None:
            randomized_noises = [
                torch.randn(self.config.train_params.batch_size, 1, int(h), int(w))
                    for (h,w) in zip(self.noise_heights, self.noise_widths)]
            self.constant_noises = randomized_noises
        else:
            randomized_noises = self.constant_noises

        self.logger.info(f"local_latent is {local_latent}")
        # self.logger.info(f"coords is {meta_coords}")
        # self.logger.info(f"noises is {randomized_noises}")
...
class InfiniteGenerationManagerFullLatent(BaseTestManager):
    ...
    def generate(self, testing_vars, tkinter_pbar=None, update_by_ss_map=None, update_by_ts_map=None, 
            ...
            g_ema_kwargs = {
                ...
                "local_latent": cur_local_latent,
                "override_coords": cur_coords,
                "noises": testing_vars.noises,
                "disable_dual_latents": True,
                "calc_flops": calc_flops,
            }
            b, c, h, w = testing_vars.meta_img.shape
            index_tuple = 0, h, 0, w
            exec_time, flops = self.maybe_parallel_inference(
                testing_vars, g_ema_kwargs=g_ema_kwargs, index_tuple=index_tuple, return_exec_time=write_gpu_time, calc_flops=calc_flops)
...
class InfiniteGenerationManagerConstantLocal(BaseTestManager):
...
class InfiniteGenerationManagerConstantCoords(BaseTestManager):
...
class InfiniteGenerationManagerPatchCoords(BaseTestManager):
    ...
    def generate(self, testing_vars, tkinter_pbar=None, update_by_ss_map=None, update_by_ts_map=None, 
        ...
        for iiter, (idx_x,idx_y) in enumerate(pbar):
            ...
            x_st = testing_vars.meta_coords[:, :, zx_st, 0]
            x_size = zx_ed - zx_st + 1
            y_st = testing_vars.meta_coords[:, :, 0, zy_st]
            ...
            if "spgan_v5" in self.config.train_params.g_arch:
...
class InfiniteGenerationManagerCloseup(BaseTestManager):
...
class InfiniteGenerationManagerCloseupZeroCoords(BaseTestManager):
    ...
    def generate(self, testing_vars, tkinter_pbar=None, update_by_ss_map=None, update_by_ts_map=None, 
        ...
        for iiter, (idx_x,idx_y) in enumerate(pbar):
            ...
            # 实现循环采样
            cur_coords = self.circular_sample_width(testing_vars.meta_coords, testing_vars.meta_coords.shape[3], zx_st, zx_ed, zy_st, zy_ed)
            cur_coords = torch.zeros_like(cur_coords).to(self.device)
            g_ema_kwargs = {
                "global_latent": testing_vars.global_latent,
                "local_latent": cur_local_latent,
                "override_coords": cur_coords,
...
class InfiniteGenerationManagerSpNormConv(BaseTestManager):
    ...
    def generate(self, testing_vars, tkinter_pbar=None, update_by_ss_map=None, update_by_ts_map=None, 
        ...
        for iiter, (idx_x,idx_y) in enumerate(pbar):
            ...
            self.const_grid_size_x = self.meta_height
            self.const_grid_size_y = self.meta_width
...
class InfiniteGenerationManagerSpNormConvZeroCoords(BaseTestManager):
    ...
    def generate(self, testing_vars, tkinter_pbar=None, update_by_ss_map=None, update_by_ts_map=None, 
        ...
        for iiter, (idx_x,idx_y) in enumerate(pbar):
            ...
            # 0:35, 0:35; 下一循环为 0:35, 6:41
            # cur_coords = testing_vars.meta_coords[:, :, zx_st:zx_ed, zy_st:zy_ed].to(self.device)
            cur_coords = torch.zeros_like(testing_vars.meta_coords[:, :, zx_st:zx_ed, zy_st:zy_ed]).to(self.device)
...
class InfiniteGenerationManagerSpNormConvCloseUp(BaseTestManager):
    ...
    def task_specific_init(self, output_size=None):
        ...
        self._init_close_up_starting_points()         
        self.noise_heights = self.outfeat_step_sizes * (self.num_steps_h-1) + self.outfeat_sizes_list
        self.noise_widths  = self.outfeat_step_sizes * (self.num_steps_w-1) + self.outfeat_sizes_list
    ...
    def _init_close_up_starting_points(self):
        # 定义重叠的尺寸
        # Get all the starting points of the patches to generate, 计算生成的小块数量
        self.num_steps_h = \
            math.ceil((self.target_height - self.outfeat_sizes_list[-1]) / self.pixelspace_step_size) + test_meta_extra_pad
...
class InfiniteGenerationManagerSpNormConvCloseUpZeroCoords(BaseTestManager):
...
class InfiniteGenerationManagerSpNormConvCloseLoop(BaseTestManager):
    ...
    def generate(self, testing_vars, tkinter_pbar=None, update_by_ss_map=None, update_by_ts_map=None, 
        ...
        for iiter, (idx_x,idx_y) in enumerate(pbar):
            ...
            # NOTE: 实现coords_partial的循环处理
            # 写coords_partial的y方向和circular_flag的处理函数
...
class InfiniteGenerationManagerPatchCoordsZeroCoords(BaseTestManager):
    ...
    def generate(self, testing_vars, tkinter_pbar=None, update_by_ss_map=None, update_by_ts_map=None, 
        ...
        for iiter, (idx_x,idx_y) in enumerate(pbar):
            ...
            cur_local_latent = testing_vars.local_latent[:, :, zx_st:zx_ed, zy_st:zy_ed].to(self.device)
            # 0:35, 0:35; 下一循环为 0:35, 6:41
            cur_coords = testing_vars.meta_coords[:, :, zx_st:zx_ed, zy_st:zy_ed]
            cur_coords = torch.zeros_like(cur_coords).to(self.device)
...
class InfiniteGenerationManagerPatchCoordsNearStep(BaseTestManagerNearStep):
